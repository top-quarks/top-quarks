...
double evaluateScore(int ci, point&dp, point&xp, point&bap) {
  ...
  if (err.y <-M_PI) err.y += M_PI*2;
  err.y *= dp.x; //Why doesn't dp.x work better than r.x?
...
double scorepathDensity(vector<int>&path) {
  ...
  //TODO: add constant multiple to quadruple and duplicate scores (order of ~1e2)
  ...
  for (int i = 0; i < path.size(); i++) {
    ...
    int k = max(j, 2);
    double s = pow(scoreDuplicateDensity(u[k-2], u[k-1], u[k], path[i]), 0.92)*dup_off;
    s = min(s, dup_max);
    prod *= s;
  }
