...
data = np.loadtxt('data2/data%d'%index)#_%d'%(ma, mb))
...
print(fp, tp)
if (tp < 30):
    os.system('rm pair_logreg%d/model%d'%(modeli, index))
    exit(0)
run = 1
if run:
    ...
    joblib.dump(logreg, 'pair_logreg%d/logreg%d.pkl'%(modeli, index))
else:
    logreg = joblib.load('pair_logreg%d/logreg%d.pkl'%(modeli, index))
...
f = open("pair_logreg%d/model%d"%(modeli, index), "w")
print(thres)
