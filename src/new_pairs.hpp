...
//Dump what is needed for fitting the logistic regression model
void trainNewPairs(vector<int>*tube) {
  const int n = 100;
  ...
  for (int i = 0; i < n; i++) {
    ...
    int goods = 0;
    char filename[100];
    sprintf(filename, "trained/tmp/data%d", i);
...
//Find pairs using the logistic regression model
vector<pair<int, int> > findPairsNew(vector<int>*tube, int modeli = 3) {
  const int features = 6;
  double feature[features];
  ...
  for (int i = 0; i < n; i++) {
    ...
    {
      char filename[100];
      sprintf(filename, "trained/pair_logreg%d/model%d", modeli, i);
...
//Output the data needed to fit a logistic regression model to stderr
void dumpPrunedTriples(vector<triple>&triples) {
